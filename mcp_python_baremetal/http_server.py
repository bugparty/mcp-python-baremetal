...
class HTTPServer:
    ...
    def _create_app(self) -> FastAPI:
        ...
        async def execute_code(request: CodeExecutionRequest):
            ...
            try:
                ...
                result = self.executor.execute(
                    request.code, auto_import=request.auto_import
                )
            ...
            async def event_generator():
                try:
                    ...
                    yield {
                        "event": "start",
                        "data": json.dumps(
                            {"status": "executing", "code": request.code}
                        ),
                    }
                    ...
                    yield {
                        "event": "error",
                        "data": json.dumps({"error": str(e), "status": "failed"}),
                    }
        ...
        async def analyze_code(request: CodeAnalysisRequest):
            ...
            try:
                missing_libs, existing_imports = self.executor.analyze_imports(
                    request.code
                )
                ...
                return {
                    ...
                    "modified_code": (
                        modified_code if modified_code != request.code else None
                    ),
...
def run_http_server(host: str = "0.0.0.0", port: int = 8000):
    ...
    uvicorn.run(http_server.app, host=host, port=port, log_level="info")
