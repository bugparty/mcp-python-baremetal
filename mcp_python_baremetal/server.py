        ...
        async def list_tools() -> List[Tool]:
            ...
            return [
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "auto_import": {
                                "type": "boolean", 
                                "description": "Whether to automatically inject missing imports",
                                "default": True
                            },
                            "reset_env": {
                                "type": "boolean",
                                "description": "Whether to reset the execution environment before running",
                                "default": False
                            }
                        },
                        "required": ["code"]
                ...
                Tool(
                    ...
                    description="Analyze Python code to show what imports would be added",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "code": {
        ...
        async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            if name == "execute_python":
                ...
                if result.modified_code:
                    response_parts.append(f"**Modified Code (auto-imports added):**\n```python\n{result.modified_code}\n```")
                    
                response_parts.append(f"**Status:** {result.execution_status} (code: {result.status_code})")
                ...
                self.executor.reset_environment()
                return [TextContent(type="text", text="Python execution environment has been reset.")]
                ...
                response_parts = []
                if existing_imports:
                    response_parts.append(f"**Existing imports:** {', '.join(sorted(existing_imports))}")
                ...
                if missing_libs:
                    ...
                    response_parts.append(f"**Missing libraries detected:** {', '.join(sorted(missing_libs))}")
                    if imports_to_add:
                        response_parts.append(f"**Would add imports:**\n```python\n" + "\n".join(imports_to_add) + "\n```")
                else:
                    response_parts.append("**No missing imports detected.**")
                    
                modified_code = self.executor.inject_imports(code)
                if modified_code != code:
                    response_parts.append(f"**Code with auto-imports:**\n```python\n{modified_code}\n```")
